...
def _format_memories_to_summarize(speaker, relevant_memories: List[Document]) -> str:
    ...
    for mem in relevant_memories:
        ...
        # person_trans = personal_transformation(all_script[speaker], mem.page_content.strip())
        # if "转换结果：" in person_trans:
        #     person_trans.split("转换结果：")[0]
        # elif "转换结果" in person_trans:
        #     person_trans.split("转换结果")[0]
        # content.append(f"{person_trans}")
...
def main():
    ...
    agent_script_rag = {}
    # 修改3：由于剧本一致性需要原剧本，所以需要修改成你评判的剧本
    play = "play6_inpc"
