...
class GenerativeAgent(BaseModel):
    ...
    def _generate_reaction(
        ...
        kwargs = dict(agent_summary_description=agent_summary_description,
                      ...
                      reasons_for_no = reasons_for_no,
                      origin_query = origin_query
                      )

        kwargs["recent_observations"] = '无\n。'
        ...
        prompt = PromptTemplate.from_template(
                ...
                + clues_str
                + "\n\n"
                + "你之前已经进行了回答：{origin_query}，但是生成的回答不符合要求，存在以下不足：{reasons_for_no}"
                + "请你参考指出的不足，在不大幅删减原有回答的基础上，结合剧本相关内容，只修改被指出不符合要求的部分。"
                + "\n\n"
                +suffix
        )
    ...
    def generate_dialogue_response_with_instrcutor(self, origin_query, observation: str, respondent: str, reasons_for_no) -> Tuple[bool, str]:
        """React to a given observation."""
        n = 0
        n+=1
        if respondent == "主角玩家":
            call_to_action_template = (
            '{agent_name}会说什么？回答问题请用以下格式： #回答#：要说的话 来回答问题。{agent_name}回答主角玩家的问题说 #回答#：\n'
        )
            full_result = self._generate_reaction(origin_query, observation,"主角玩家", call_to_action_template, reasons_for_no)
        else:
            call_to_action_template = (
            '{agent_name}会说什么？回答问题请用以下格式： #回答#：要说的话 来回答问题。{agent_name}回答%s的问题说 #回答#：\n'%respondent.name
        )
            full_result = self._generate_reaction(origin_query, observation,respondent.name, call_to_action_template, reasons_for_no)
    ...
    def generate_question_with_instructor(self, origin_query, observation: str, respondent: str, reasons_for_no) -> Tuple[bool, str]:
        """React to a given observation."""
        call_to_action_template = (
    ...
    def _take_action_from_choice(self,action):
        def select_ask(self):
            ...
            prompt = ChatPromptTemplate(
                messages=[
                    HumanMessagePromptTemplate.from_template("""根据你的人物故事情节：{story}。并依据你的个人任务:{mission}，以及之前游戏中你目击的和{player_to_ask}相关的信息：{context_str}。
                    请说出你想要问{player_to_ask}的问题。
                    你之前已经进行了提问，但是提出的问题不符合要求，存在以下不足：{reasons_for_no}，请你参考指出的不足以及剧本相关内容重新提问。
                    \n{format_instructions}\n""")
                ],
    ...
    def _generate_fd_questions_with_instructor(self,origin_query, player_to_ask, reasons_for_no) -> Tuple[str, str]:
        ...
        prompt = ChatPromptTemplate(
            messages=[
                HumanMessagePromptTemplate.from_template("""根据你的人物故事情节：{story}。并依据你的个人任务:{mission}，以及之前游戏中你目击的和{player_to_ask}相关的信息：{context_str}。
                请说出你想要问{player_to_ask}的问题。
                你之前已经进行了提问：{origin_query}，但是提出的问题存在以下不足：{reasons_for_no}，请你参考指出的不足，在不大幅删减原有提问的基础上，结合剧本相关内容重新提问。
                \n{format_instructions}\n""")
            ],
            input_variables=["story", "mission","player_to_ask","context_str"],
            partial_variables={"format_instructions": format_instructions}
        ...
        _input = prompt.format_prompt(origin_query = origin_query,reasons_for_no = reasons_for_no, story=self.character_story, mission=self.mission, player_to_ask=player_to_ask, context_str = context_str)
        ...
        return remove_quotes_and_colons(question_to_ask)
...
def self_introduction(agents: List[GenerativeAgent], victim: String) -> None: # type: ignore
...
if __name__ == "__main__":
    ...
    for i in range(len(data['角色'])):
        agents[data['角色'][i]['角色名']] = GenerativeAgent(name=data['角色'][i]['角色名'],
                      age=data['角色'][i]['年龄'],
                      role=data['角色'][i]['角色'],
    ...
    # record_agent(folder_path=path_to_save_agents,agents= list(agents.values()),phase='r0')



    print('****************反思开始*******************\n')

    with open('z_reflexion_style2.json', encoding='utf-8') as f:
    ...
    for neg in negative_data:
        ...
        if neg['提问人'] == "主角玩家":
            reason = neg['原因']
            content = neg['提问内容']
            origin_answer = neg['回答']
            _, new_answer = agent.generate_dialogue_response_with_instrcutor(origin_query = origin_answer, observation = content, respondent = "主角玩家", reasons_for_no = reason)
            print(f"原来的回答：{origin_answer}\n新的回答：{new_answer}")
            global_output_list.append(f"\n（问题）主角玩家对{agent.name}说：\"{content}\"\n\n\n（回答）{agent.name}对主角玩家说：\"{new_answer}\"")
        else:
            interviewer = agents[neg['提问人']]
            reason = neg['原因']
            content = neg['提问内容']
            origin_answer = neg['回答']
            _, new_answer = agent.generate_dialogue_response_with_instrcutor(origin_query = origin_answer, observation = content, respondent = interviewer, reasons_for_no = reason)
            print(f"原来的回答：{origin_answer}\n新的回答：{new_answer}")
            global_output_list.append(f"\n（问题）{interviewer.name}对{agent.name}说：\"{content}\"\n\n\n（回答）{agent.name}对{interviewer.name}说：\"{new_answer}\"")
